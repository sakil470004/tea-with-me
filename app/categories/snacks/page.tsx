...
export default function AllSnacksPage() {
  ...
  const fetchSnacksProducts = async () => {
    try {
      const response = await fetch('/api/products?category=snacks');
      if (response.ok) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="animate-pulse">
            <div className="h-12 bg-purple-200 rounded-lg mb-8 w-64 mx-auto"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600 mb-4">
            🍪 Delicious Snacks Collection
          </h1>
          <p className="text-lg text-purple-700 max-w-2xl mx-auto">
            Indulge in our carefully selected snacks and treats. Perfect companions for your tea and coffee moments.
          </p>
          <div className="mt-6 text-sm text-purple-600">
            Showing {products.length} snack{products.length !== 1 ? 's' : ''}
          </div>
        </div>
        ...
        {products.length === 0 ? (
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold text-purple-700 mb-2">No snacks found</h3>
            <p className="text-purple-600">Our snacks collection is currently being updated.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {products.map((product) => (
              <Link href={`/products/${product._id}`} key={product._id}>
                <div className="bg-white rounded-lg shadow-md hover:shadow-xl transition-all duration-300 group cursor-pointer border border-purple-100 hover:border-purple-300">
